...
import { GitPullRequestCreate, Pencil, Star } from "lucide-react";
...
export default function CreateAdventureDialog() {
    ...
    const handleCreate = async (values: CreateAdventureFormValues) => {
        setIsCreating(true);
        try {
            await createAdventure.mutateAsync(values as AdventureDTO);
            setOpen(false);
        } finally {
            setIsCreating(false);
        }
            ...
            <AlertDialogContent className="rounded-lg p-6 md:max-w-3xl">
                <AlertDialogHeader>
                    ...
                    <AlertDialogDescription className="text-muted-foreground text-sm mt-2">
                        Turn a moment into an adventure.
                    </AlertDialogDescription>
                </AlertDialogHeader>
